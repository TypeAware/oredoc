...
//     packageName = getPackageName(v[symbols.generic.Parent]) + '/' + fileName
//   }
//
//
...
//           if (rhs && typeof rhs === 'object') {
//
//             if (!rhs[symbols.generic.TypeMap] && circularRefCache.has(rhs)) {
//               // throw new Error(
...
//
//             if (!startFile) {
//               // throw new Error('We are not within a .go file, so we cannot handle a non-object value yet => ' + v);
//             }
//
//             const val = getString(rhs);
//
...
//           startFile = startFile || rhs[symbols.lang.go.File] === true;
//           startEntity = startEntity || rhs[symbols.lang.go.Entity] === true;
//
//
...
//     },
//     (err, results) => {
//       console.log({err, results: results.length});
//
//       cb(err);
//
//     });
...
// const src = '/home/oleg/codes/oresoftware/types-depot/tc.input.js';
// const {tc} = require(src);
// const input = tc.entitiesMap.get('default');
//
// console.log({input});
//
// generateToFiles(root,input, (err: any) => {
//     console.error({err});
// });
//
